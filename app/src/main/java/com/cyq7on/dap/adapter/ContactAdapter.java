...
public class ContactAdapter extends BaseRecyclerAdapter<Friend> {
    ...
    public void bindView(BaseRecyclerHolder holder, Friend friend, int position) {
        if(holder.layoutId== com.cyq7on.dap.R.layout.item_contact){
        ...
        }else if(holder.layoutId== com.cyq7on.dap.R.layout.header_new_friend){
            if(NewFriendManager.getInstance(context).hasNewFriendInvitation()){
                holder.setVisible(com.cyq7on.dap.R.id.iv_msg_tips,View.VISIBLE);
            }else{
                holder.setVisible(com.cyq7on.dap.R.id.iv_msg_tips, View.GONE);
            }
        }
    }
